...
- **Aligned transcript**: where every spoken chunk is attached to the slide that was on screen when it was said.
...
- **Media import**: transcribe an existing video or audio file instead of recording live. Scrub to a starting point and import with the same OCR + transcription pipeline.
- **Capture-region overlay**: when using "Mouse Select" capture mode, a full-screen overlay lets you drag to draw the recording region before capture starts.
- **Capture countdown**: a real-time countdown in the transcript panel footer shows seconds until the next slide capture.
- **UI scaling**: enlarge or shrink the entire interface (90 %–125 %) from Settings → Appearance. Applies on restart.
- **Hardware detection**: probe your GPU/CPU to get a recommended speech model and download it with one click.
- **API connection test**: test your Gemini API key against every model the app uses and see per-model quota status.
...
| Cloud AI | google-genai (Google Gemini for OCR vision, speech, summary, quiz, translate/define) |
...
```
The runnable app is `dist\LectureCapture\`. This is the `LectureCapture.exe` plus its `_internal\` folder. To share it, zip that whole folder (the .exe needs `_internal\` beside it).
