...
class TranscriptPanel(QWidget):
    ...
    def __init__(self, base_dir, icons_dir) -> None:
        ...
        # Pause/Resume an active recording. Sits between Capture Now and Record. Only shown while recording.
        ...
        next_cap_w, self.next_capture_label = create_label(icons_dir / 'timer.svg', '')
        self.next_capture_widget = next_cap_w
        ...
        # Hide the countdown widget by default; only shown during live recording.
        self.set_next_capture_countdown(None)
    ...
    def set_next_capture_countdown(self, seconds: float | None) -> None:
        """Show time until the next capture interval. None/negative hides the widget."""
        if seconds is None or seconds <= 0:
            self.next_capture_label.setText("")
            self.next_capture_widget.setVisible(False)
        else:
            self.next_capture_label.setText(f"{seconds:.1f}s")
            self.next_capture_widget.setVisible(True)
